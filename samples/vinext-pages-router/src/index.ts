...
// Set up vinext globals for client-side hydration
// Find the client entry file from the manifest
let clientEntryFile: string | null = null;
for (const [key, value] of Object.entries(manifest)) {
  if (key.includes('vinext-client-entry') || (value as any).name === '_virtual_vinext-client-entry') {
    clientEntryFile = (value as any).file;
    break;
  }
}

// Build SSR manifest (maps source module paths to their output assets)
const ssrManifest: Record<string, string[]> = {};
for (const [key, value] of Object.entries(manifest)) {
  const entry = value as any;
  if (entry.src) {
    ssrManifest[entry.src] = [entry.file];
  }
}

// Inject globals that vinext's renderPage expects
if (clientEntryFile) {
  (globalThis as any).__VINEXT_CLIENT_ENTRY__ = clientEntryFile;
}
...
async function handleRequest(request: Request): Promise<Response> {
  try {
    ...
    if (typeof runMiddleware === "function") {
      // TODO: Once we have middlewares, we should pass the request as an argument
      const result = await runMiddleware();

      if (!result.continue) {
        // TODO: default sample has no middleware so this will always continue
        // but once there is middleware, we should handle appropriately
      }
    }
    ...
    if (configHeaders.length) {
      const matched = matchHeaders(resolvedPathname, configHeaders, {} as RequestContext);
